// MIG Cyber Policy Issuance — app logic. Deployed as admin/cyber/policy/policy_app.js
/* global supabase, docx, html2canvas, jspdf, CYBER_WORDING, SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY */
...
function renderPreview() {
  ...
  h += '<h1 class="t">ПОЛИС ПО КОМПЛЕКСНОМУ СТРАХОВАНИЮ КИБЕРРИСКОВ</h1><div class="tsub">страховой полис является Договором Страхования</div>';
  h += '<h1 class="t">CYBER INSURANCE POLICY</h1><div class="tsub">This Insurance Policy acts as an Insurance Agreement (contract)</div>';
  ...
  // Wording
  h += '<h2 class="s" style="margin-top:26px">ПРАВИЛА (УСЛОВИЯ) КОМПЛЕКСНОГО СТРАХОВАНИЯ КИБЕРРИСКОВ<br/>CYBER INSURANCE TERMS AND CONDITIONS</h2>';
  h += '<table class="wrd">';
  ...
  h += '</table>';
  h += '<div class="foot">MIG Cyber Protect Wording v1.0 (2026) / Правила МИГ по страхованию киберрисков, ред. 1.0 (2026)</div>';
  h += '</div>';
...
async function generateDocx() {
  ...
  ch.push(heading("ПОЛИС ПО КОМПЛЕКСНОМУ СТРАХОВАНИЮ КИБЕРРИСКОВ", { align: AlignmentType.CENTER, size: 28, before: 120 }));
  ch.push(para("страховой полис является Договором Страхования", { align: AlignmentType.CENTER, size: 17, after: 120 }));
  ch.push(heading("CYBER INSURANCE POLICY", { align: AlignmentType.CENTER, size: 28, before: 40 }));
  ...
  ch.push(para("MIG Cyber Protect Wording v1.0 (2026) / Правила МИГ по страхованию киберрисков, ред. 1.0 (2026).", { size: 15, align: AlignmentType.CENTER, color: "888888" }));
...
async function loadSources() {
  ...
  const { data: quotes } = await sb.from('cyber_quotes')
...
async function loadAndFill() {
  ...
  if (loadedSubmissionId) {
    ...
    if (!error && data) {
      ...
      // Territory from geography checkboxes (both naming patterns)
      const g = k => f['cyber_geo_' + k] === true || f['cyber_geo_' + k] === 'true' || f['cy_geo_' + k] === true || f['cy_geo_' + k] === 'true';
      const geo = [];
      if (g('uz')) geo.push('UZ');
      if (g('cis')) geo.push('CIS');
      if (g('eu')) geo.push('EU');
      if (g('us')) geo.push('US');
      if (g('worldwide')) geo.push('WW');
  ...
  if (loadedQuoteId) {
    const { data, error } = await sb.from('cyber_quotes').select('id, insured_name, final_premium, inputs').eq('id', loadedQuoteId).single();
    if (!error && data) {
      if (!$('f-insured').value) setVal('f-insured', data.insured_name, 'pf-name');
      setVal('f-limit', data.inputs.limit, 'pf-limit');
      setVal('f-retention', data.inputs.retention, 'pf-ret');
...
async function confirmIssue() {
  ...
  let resp;
  if (policyRecordId) {
    resp = await sb.from('cyber_policies').update(payload).eq('id', policyRecordId).select('id').single();
  } else {
    resp = await sb.from('cyber_policies').insert(payload).select('id').single();
  }
...
(async function init() {
  ...
  ['f-limit','f-retention','f-premium','f-sl-ext','f-sl-osp','f-sl-fines','f-sl-tel','f-sl-cj'].forEach(id => bindThousands($(id)));
