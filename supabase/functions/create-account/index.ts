...
//   create : { account_type:'customer'|'staff', email, password?, full_name?, company?, role? }
//            -> { user_id, email, password, account_type }
//   list   : {} -> { users: [ {user_id,email,type,role,full_name,company,created_at,last_sign_in_at,email_confirmed_at} ] }
//   update : { user_id, full_name?, company?, role?, email?, password? } -> { ok:true }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "list") {
      ...
      const { data: custs } = await admin.from("customers").select("user_id, email, full_name, company");
    ...
    if (action === "update") {
      ...
      if (typeof body.email === "string" && body.email.includes("@")) authPatch.email = body.email.trim().toLowerCase();
    ...
    if (account_type === "staff") {
      ...
      const { error: pErr } = await admin.from("customers").insert({ user_id: newId, email, full_name, company, created_by: user.id });
